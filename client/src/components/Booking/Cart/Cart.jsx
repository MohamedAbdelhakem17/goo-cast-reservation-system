...
function StudioSection({ studio, date, startSlot, formatTime, duration }) {
    ...
    return (
        <div className="space-y-1 border-b-1 border-gray-300 pb-3">
            <p className="text-sm text-gray-500"><i className="fa-solid fa-calendar-days mr-2 text-[12px]"></i>{formatDate(date)}</p>
            <p className="text-sm text-gray-500"><i className="fa-solid fa-clock mr-2 text-[12px]"></i>{formatTime(startSlot)} ({duration}h) </p>
            <p className="text-sm text-gray-500"><i className="fa-solid fa-location-dot mr-2 text-[12px]"></i>{studio.name}</p>
        </div>
    );
}
...
function PackageSection({ selectedPackage, duration, totalPackagePrice, priceFormat }) {
    ...
    return (
        <div className="flex items-center justify-between pb-1 pt-2">
            <p className="text-sm text-gray-500">{selectedPackage.name} ({duration}h)</p>
        ...
        <ul className="border-b-1 border-gray-300 pb-2">
            {selectedAddOns.map((addon) => (
                <li className="flex items-center justify-between  py-1" key={addon._id}>
                    ...
                    <p className="text-sm text-gray-500"> x{addon.quantity} / {addon.price} EGP</p>
                </li>
            ))}
        </ul>
    );
...
export default function Cart() {
    ...
    const totalAddOnPrice = useMemo(() => {
        return (
            bookingData.selectedAddOns?.reduce(
                (acc, item) =>
                    acc + (item.quantity > 0 ? item.price * item.quantity : 0),
                0
            ) || 0
        );
    ...
    return (
        <div className="border rounded-lg border-gray-200 py-4 px-5">
            ...
            {Boolean(
                ...
                bookingData.totalPriceAfterDiscount !== 0
            ) && (
