export default function WhatsAppButton({ phone = "201010955331" }) {
  ...
  return (
    <div className="group fixed right-2 bottom-36 z-50 md:right-4 md:bottom-4">
      <a
        href={href}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat on WhatsApp"
        className="flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] shadow-lg transition-transform duration-200 hover:scale-110 hover:shadow-xl"
      >
        <svg
          viewBox="0 0 24 24"
      ...
      {/* Tooltip */}
      <span className="pointer-events-none absolute right-16 bottom-1/2 translate-y-1/2 rounded-md bg-gray-800 px-3 py-1 text-sm text-white opacity-0 shadow-md transition-opacity duration-200 group-hover:opacity-100">
        تواصل معنا
      </span>
    </div>
  );
