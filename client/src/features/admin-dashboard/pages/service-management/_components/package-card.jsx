...
import { Check, CheckCircle, Edit2, FileText, List, Trash, Users } from "lucide-react";

export default function PackageCard({ pkg, setSelectedDeletedPackage }) {
  // Localization
  ...
  return (
    <div className="flex flex-col justify-between gap-y-4 overflow-hidden rounded-lg bg-white p-6 shadow-md transition-shadow hover:shadow-lg">
      {/* Header */}
            ...
            <div className="space-y-1.5">
              {pkg.post_session_benefits[lng].map((benefit, index) => (
                <div key={index} className="flex items-start gap-2">
                  <Check className="mt-1 h-3 w-3 flex-shrink-0 text-slate-500" />
      ...
      <div className="mt-auto flex items-center justify-center gap-2 border-t border-slate-200 pt-4">
        ...
        >
          <Trash className="h-4 w-4 cursor-pointer text-red-500 group-hover:text-red-600" />
        </button>
