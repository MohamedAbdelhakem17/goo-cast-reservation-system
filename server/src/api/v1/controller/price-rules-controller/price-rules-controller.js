...
exports.getAllPriceRules = asyncHandler(async (req, res, next) => {
  ...
  if (priceRules.length === 0) {
    return next(
      new AppError(404, HTTP_STATUS_TEXT.FAIL, "No price rules found"),
    );
  }
...
exports.addNewPriceRule = asyncHandler(async (req, res, next) => {
  ...
  if (!rules || !Array.isArray(rules) || rules.length === 0) {
    return next(
      new AppError(400, HTTP_STATUS_TEXT.FAIL, "Rules array is required"),
    );
  }
    ...
    if (!isFixedHourly && !perSlotDiscounts) {
      return next(
        new AppError(
          ...
          "per Hour Discounts required when not fixed hourly",
        ),
      );
    }
    ...
    if (exists) {
      return next(
        new AppError(
          400,
          HTTP_STATUS_TEXT.FAIL,
          `Price rule already exists for day ${dayOfWeek}`,
        ),
      );
    }
  }
...
exports.editPriceRule = asyncHandler(async (req, res, next) => {
  ...
  const priceRule = await PriceRuleModel.findOneAndUpdate(
    { package, dayOfWeek },
